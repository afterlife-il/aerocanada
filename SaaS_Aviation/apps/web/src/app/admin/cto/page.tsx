  ...
  return (
    <details className="rounded border border-border bg-panel">
      <summary className="grid cursor-pointer list-none gap-3 p-3 md:grid-cols-[minmax(12rem,1.2fr)_8rem_1fr_minmax(12rem,1fr)]">
        ...
        <span className={`h-fit w-fit rounded px-2 py-1 text-xs font-semibold ${tone[module.status]}`}>{module.controlStatus}</span>
      ...
      <div className="grid gap-4 border-t border-border p-4 xl:grid-cols-2">
        <section className="xl:col-span-2"><h3 className="mb-2 font-semibold">Completion dimensions</h3><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{Object.entries(module.dimensions).map(([label, value]) => <div className="rounded border border-border bg-panel-muted p-2" key={label}><div className="text-xs text-muted">{label}</div><div className="mt-1 text-lg font-semibold">{value}%</div></div>)}</div></section>
