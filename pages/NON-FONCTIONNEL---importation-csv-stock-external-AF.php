<?php
$fop = fopen('csv/AF_surplus_list.csv', 'r');
if($fop === false)
{
...
//Aircraft;Description;MPN;MSN;Annotation;Quantity;Condition;DteMiseSvc;DOM;Dte Last Repair;Dte Last OH;Dte Last Bench;TSN;TSO;TSR;TSB;ATA;SATA;Code UN;Acr Last removal
		...
		if ($varoy=='8') $DteMiseSvc=$val;
		if ($varoy=='9') $DOM=$val;
		if ($varoy=='10') $Dte_Last_Repair=$val;
		if ($varoy=='11') $Dte_Last_OH=$val;
		if ($varoy=='12') $Dte_Last_Bench=$val;
		if ($varoy=='13') $TSN=$val;
		if ($varoy=='14') $TSO=$val;
		if ($varoy=='15') $TSR=$val;
		if ($varoy=='16') $TSB=$val;
		if ($varoy=='17') $ATA=$val;
		if ($varoy=='18') $SATA=$val;
		
		$Fld_Stock_Remark ="Aircraft :".$Aircraft;

							//model avion 	
							$Fld_AC_ID='';
							//Fin model avion 	
							//COMPANY Fld_Company_ID
							$Fld_Company_ID="2855";
							//FIN COMPANY Fld_Company_ID
								...
								//****************************************************			
								//verif si le modele d'avion se trouve dans notre base
								if (!empty($Category))
								{
											//Fld_AC_ID  Fld_AC_Model  Fld_AC_Series  Fld_AC_Manufacturer  Fld_AC_Engine_Model  Fld_AC_Engine_Series
											$sqlairc="SELECT * FROM tbl_Aircraft where Fld_AC_Model='".$Category."'";
											
											$resultairc = mysql2_query($sqlairc);
											$nb_resultats_airc = mysqli_num_rows($resultairc);
					                        //Fin Verification si le pn se trouve dans la table tbl_Parts
											$rowairc = mysqli_fetch_array($resultairc);
											
											if (0<$nb_resultats_airc) $Fld_AC_ID=$rowairc['Fld_AC_ID'];
											else
											{
												$reqcairc="INSERT INTO `tbl_Aircraft` (`Fld_AC_ID`, `Fld_AC_Model`, `Fld_AC_Series`, `Fld_AC_Manufacturer`, `Fld_AC_Engine_Model`, `Fld_AC_Engine_Series`) VALUES (NULL, '".$Category."', NULL, NULL, NULL, NULL);";
												$requetecairc = mysql2_query($reqcairc);
												$Fld_AC_ID=mysqli_insert_id($connection);
											}
								}
								//Fin verif si le modle d'avion se trouve dans notre base
								//****************************************************
								...
								$req="INSERT INTO `tbl_Stock_external` (`Fld_Stock_externe_ID`, `Fld_Part_ID`, `Fld_Part_SN`, `Fld_Supplier_ID`, `Fld_Entry_Date`, `Fld_Part_Price`, `Fld_Price_Currency_ID`, `Fld_BAX_PO_Nbr`, `Fld_Supplier_order_Date`, `Fld_Supplier_Payment_Date`, `Fld_Qty`, `Fld_Condition_ID`, `Fld_Release_ID`, `Fld_Tag_Info_ID`, `Fld_Tag_Date`, `Fld_Traceability_ID`, `Fld_Warehouse_Location`, `Fld_Physical_Stock`, `Fld_Owner_ID`, `Fld_Stock_Location_ID`, `Fld_Status_ID`, `Fld_Status_Ind`, `Fld_Status_Date`, `Fld_Stock_Remark`, `Fld_Shelf_Life_Limit`, `Fld_Valeur_Comptable`, `Fld_Valeur_Comptable_currency_Id`, `Fld_Sales_Remark`, `Fld_External_Location`, `Fld_Sales_Remark_ID`, `Fld_Warehouse_Location_ID`, `Fld_OriginalUnit_Stock_ID`, `Fld_Min_Qty`, `Fld_Publish`, `status`, `Fld_AC_ID`, `Fld_Company_ID`, `DteMiseSvc`, `DOM`, `Dte_Last_Repair`, `Dte_Last_OH`, `Dte_Last_Bench`, `TSN`, `TSO`, `TSR`, `TSB`, `ATA`, `SATA`) VALUES ('', '".$Fld_Part_ID."', '".$SN."', NULL, '".$today."', NULL, NULL, NULL, NULL, NULL, '".$QTY."', '".$Fld_Condition_ID."', NULL, NULL, '".$DATE."', NULL, NULL, NULL, NULL, '15', NULL, NULL, NULL, '".$Fld_Stock_Remark."', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '', '".$Fld_AC_ID."', '".$Fld_Company_ID."', '".$DteMiseSvc."', '".$DOM."', '".$Dte_Last_Repair."', '".$Dte_Last_OH."', '".$Dte_Last_Bench."', '".$TSN."', '".$TSO."', '".$TSR."', '".$TSB."', '".$ATA."', '".$SATA."');"; 
