                ...
                <div class="col-lg-10">
                   <div class="panel panel-default">
                        <div class="panel-heading">
                            ADDRESS TYPE<a href="javascript:add_address_type()"> + Add A ADDRESS TYPE</a>
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                            <div class="table-responsive">
							<form action='validation_address_type.php' method="post">
                                <table class="table table-striped table-bordered table-hover" id="mytable">
									...
									$objet=new company();
									$donnee = $objet->affichage_address_type();
									foreach($donnee as $dataemp)
									{
									$z++;
									$varsuivante=$z+1;
								
                                       echo "<tr class=\"odd gradeX\" id=\"row_".$dataemp["Fld_Division_ID"]."\"><td><b>".$dataemp["Fld_Division_ID"]."</b></td><td><input type='text' name='Fld_Division_Text' value='".$dataemp['Fld_Division_Text']."' id='Fld_Division_Text".$dataemp["Fld_Division_ID"]."' onmouseleave='javascript:majtarea(".$dataemp["Fld_Division_ID"].")'><input type=\"hidden\" name=\"nbaddtype\" id=\"nbaddtype\" value=\"".$z."\"></td>";
									   echo "<td><a href='javascript:sup_address_type(".$dataemp["Fld_Division_ID"].",".$z.")' onClick=\"return(confirm('Etes vous sur ?'));\"><img src='images/bin-blue-full-icon.png' border='0' width='27'></a></td>";
									   echo "</tr>";
									}?>       
                                    </tbody>
                                </table>
...
<!--Add address type-->
    ...
    //cell.innerHTML = "Ligne " + nbLignes + " Cellule 0";
    cell.innerHTML = "<input class=\"form-control\" name=\"Fld_Division_Text\" id=\"Fld_Division_Text\" placeholder=\"Address Type\"><input type='hidden' name='nbaddtype' value='"+nbLignes+"'>";
	
	cell = ligne.insertCell(2);
    cell.innerHTML = "<input type='submit' value='submit' class=\"form-control\">";
}
<!--End add address type-->
<!--Sup address type-->
function sup_address_type(id,nbligne){
        if (id > 0) {
            ...
            $.get('del_address_type.php', { // lien de la page qui permet la suppression
...
<!--Fin Supp address type-->
function majtarea(id){
 
    var selection = document.getElementById("Fld_Division_Text"+id).value;
    $.get('majaddtype.php', { // lien de la page qui permet la suppression
                Fld_Division_ID:id,Fld_Division_Text:selection //variable de type GET (on recuperera la variable avec $_GET['idsup'])
            }, function(data){
