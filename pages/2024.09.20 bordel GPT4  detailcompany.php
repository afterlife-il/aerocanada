...
// Vérification de la présence de l'ID de la compagnie
$id_company = isset($_GET['id']) ? intval($_GET['id']) : null;
if (empty($id_company)) {
    die("Erreur : Aucun identifiant de compagnie n'a été fourni.");
}
...
$reqcomn = mysqli_query($conn, $sqlcomn);
if (!$reqcomn) {
    die("Erreur SQL : " . mysqli_error($conn));
}
$datacn = mysqli_fetch_assoc($reqcomn);
$companyname = strtoupper($datacn['Fld_Company_Name']);
$customer_payment_term_id = strtoupper($datacn['customer_payment_term_id']);
$customer_payment_term_amount = strtoupper($datacn['customer_payment_term_amount']);
$customer_payment_term_currencyid = strtoupper($datacn['customer_payment_term_currencyid']);
$aci_payment_term_id = strtoupper($datacn['aci_payment_term_id']);
$aci_payment_term_amount = strtoupper($datacn['aci_payment_term_amount']);
$aci_payment_term_currencyid = strtoupper($datacn['aci_payment_term_currencyid']);
...
$reqrdpc = mysqli_query($conn, $sqlrdpc);
if (!$reqrdpc) {
    die("Erreur SQL : " . mysqli_error($conn));
}
...
$requete = "INSERT INTO tbl_maakav_company (id_maakav_company, id_company, datecomplete, heurevisite, id_Employee) 
            VALUES (NULL, $id_company, '$today', '$heuretoday', " . intval($_SESSION['id_utilisateur']) . ")";
if (!mysqli_query($conn, $requete)) {
    die("Erreur lors de l'enregistrement : " . mysqli_error($conn));
}
?>
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                ...
                <br>
                <a href='modif_company.php?Fld_Company_ID=<?php echo $id_company; ?>' style='decoration:none;color:white;' title='Modification Company'>
                    <i style='margin-left:10px;position: relative;top: 4px;font-size:23px;color:white;' class='fa fa-pencil-square-o'></i>
                </a>
                <a href='ajout_contact_company.php?Fld_Company_ID=<?php echo $id_company; ?>' style='decoration:none;color:white;' title='Add Contact Company'>
                    <img src='images/add_contact_w.png' width='28'>
                </a>
                <a href='archive_company.php?Fld_Company_ID=<?php echo $id_company; ?>' onClick="return(confirm('Etes vous sur ?'));" style='decoration:none;color:white;'  title='Archive Company'>
                ...
                <?php if ($id_company == '1182') { ?>
                    <a href='http://aerocanada-industries.com/adminaero/pages/price_list.php?Fld_Company_ID=<?php echo $id_company; ?>' style='decoration:none;color:white;'>
            ...
            <div class="panel-body">
                <div class="panel-group" id="accordion">
                    ...
                    <!-- Code pour afficher les informations générales de la société -->
                    <?php
                    ...
                    if ($req && mysqli_num_rows($req) > 0) {
                        ...
                        if (!empty($data['Fld_Company_BAX_Contact'])) {
                            $sqlemp = "SELECT Employee_Name FROM tbl_Employee WHERE Employee_ID = " . intval($data['Fld_Company_BAX_Contact']);
                            $reqemp = mysqli_query($conn, $sqlemp);
                            if ($reqemp) {
                                $dataemp = mysqli_fetch_assoc($reqemp);
                            } else {
                                $dataemp = ['Employee_Name' => 'Unknown'];
                            }
                        } else {
                            $dataemp = ['Employee_Name' => 'Unknown'];
                        }
                        ...
                        echo "<div class='form-group'><label>Company Name</label><input class=\"form-control\" type='text' name='Fld_Company_Name' value='" . htmlspecialchars($datacn['Fld_Company_Name'], ENT_QUOTES) . "'></div>";
                        ...
                        echo "</select></div>";
                        echo "</div>";
                        // Affichage du reste du formulaire, etc...
                    } else {
                        echo "Pas de réponse.";
                    }
                    ?>
...
if (!empty($data['Fld_Company_BAX_Contact'])) {
    ...
    $dataemp = ['Employee_Name' => 'Unknown']; // Par défaut si aucun employé n'est défini
}
...
echo "<div class='form-group'><label>ACI 770 Contact</label>";
echo "<select class=\"form-control\" name=\"Employee_ID\">";
// Ici vous pouvez lister les employés ou gérer la sélection
...
<div class="table-responsive">
					...
					{ 
						...
						$tel = mb_ereg_replace('[^0-9]', '', $tel); // supression sauf chiffres
					...
					echo "<td ".$statustab.">";
											...
											//echo $sqldiv;
											$reqemp = mysql2_query($sqldiv);
											$datadiv = mysqli_fetch_assoc($reqemp);
					...
					echo "<td ".$statustab.">Entry date : ".$data['entry_date']."<br><textarea class=\"form-control\" name='Fld_Contact_Remark".$data['id_company_contact']."' style='WIDTH: 400px; height:50px;'  id='recupmessageremark".$data['id_company_contact']."' onmouseleave='javascript:majtarea(".$data['id_company_contact'].")'>".$data['Fld_Contact_Remark']."</textarea></td>";
...
<div class="table-responsive">
					...
					echo "<td ".$statustab."><input class=\"form-control\" type='text' name='Fld_Contact_Phone".$x."' value='".$data['Fld_Contact_Phone']."'> <a id='myLink' href='#' onclick='callclient(".$tel.",".$dataiue['numformat'].");return false;'><i style='margin-left:10px;position: relative;top: 4px;font-size:23px;' class='fa  fa-phone'></i></a></td>";
					
					echo "<td ".$statustab."><input class=\"form-control\" type='text' name='Fld_Contact_Phone2".$x."' value='".$data['Fld_Contact_Phone2']."'>
					<input type=\"button\" id=\"confirmcall\" value=\"Register Now\" class=\"button\">  
					</td>";
...
?>
<a href="company-fiche.php?id=<?php echo $id_company;?>&actcompet=addcomp"> + Add A COMPETITOR</a>
			...
			<input type="hidden" name="id_company" value="<?php echo $id_company;?>">
						...
						<?php
											...
											while($datadacompany = mysqli_fetch_assoc($reqdacompany)){
												echo "<tr><td><a href='../docsattachmentcompany/".$datadacompany['docs_name']."' target='_blank'>".$datadacompany['name']."</a></td><td><a href='del_doc_company.php?id_docs_attachment_company=".$datadacompany['id_docs_attachment_company']."&id=".$_GET['id']."'  onClick=\"return(confirm('Are you sure ?'));\"><img src='images/bin-blue-full-icon.png' border='0' width='27'></a></td></tr>";
											}
						?>
						</table>
