<?php
$fop = fopen('csv/stock-externe-srtechnics-251018.csv', 'r');
if($fop === false)
{
...
//PN;Description;Cond;Status of Work;Tagged by;Tag Date;Geoloc.;A/C;Serial No.

		if ($varoy=='1') $Fld_Part_Nbr=$val;
		if ($varoy=='2') $FLD_PART_DESC=$val;
		if ($varoy=='3') $COND=$val;
		if ($varoy=='4') $Status=$val;
		if ($varoy=='5') $Tagged_by=$val;
		if ($varoy=='6') $Tag_Date=$val;
		if ($varoy=='7') $Geoloc=$val;
		if ($varoy=='8') $aircraft=$val;
		if ($varoy=='9') $Serial_No=$val;
		
		

		
		$Fld_Stock_Remark ="";
		if(!empty($Status)) $Fld_Stock_Remark .="Status of Work :".$Status;
		if(!empty($Tagged_by)) $Fld_Stock_Remark .=" ** Tagged by :".$Tagged_by;
		if(!empty($Tag_Date)) $Fld_Stock_Remark .=" ** Tag Date :".$Tag_Date;
		if(!empty($Geoloc)) $Fld_Stock_Remark .=" ** Geoloc. :".$Geoloc;
		if(!empty($aircraft)) $Fld_Stock_Remark .=" ** Aircraft :".$aircraft;
		if(!empty($Serial_No)) $Fld_Stock_Remark .=" ** Serial No. :".$Serial_No;
		$QTY="";					
							//COMPANY Fld_Company_ID -- SR TECHNICS SWITZERLAND
							$Fld_Company_ID="572";
							//FIN COMPANY Fld_Company_ID
								...
								$req="INSERT INTO `tbl_Stock_external` (`Fld_Stock_externe_ID`, `Fld_Part_ID`, `Fld_Part_SN`, `Fld_Supplier_ID`, `Fld_Entry_Date`, `Fld_Part_Price`, `Fld_Price_Currency_ID`, `Fld_BAX_PO_Nbr`, `Fld_Supplier_order_Date`, `Fld_Supplier_Payment_Date`, `Fld_Qty`, `Fld_Condition_ID`, `Fld_Release_ID`, `Fld_Tag_Info_ID`, `Fld_Tag_Date`, `Fld_Traceability_ID`, `Fld_Warehouse_Location`, `Fld_Physical_Stock`, `Fld_Owner_ID`, `Fld_Stock_Location_ID`, `Fld_Status_ID`, `Fld_Status_Ind`, `Fld_Status_Date`, `Fld_Stock_Remark`, `Fld_Shelf_Life_Limit`, `Fld_Valeur_Comptable`, `Fld_Valeur_Comptable_currency_Id`, `Fld_Sales_Remark`, `Fld_External_Location`, `Fld_Sales_Remark_ID`, `Fld_Warehouse_Location_ID`, `Fld_OriginalUnit_Stock_ID`, `Fld_Min_Qty`, `Fld_Publish`, `status`, `Fld_AC_ID`, `Fld_Company_ID`) VALUES ('', '".$Fld_Part_ID."', NULL, NULL, '".$today."', NULL, NULL, NULL, NULL, NULL, '', '".$Fld_Condition_ID."', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '".$Fld_Stock_Remark."', NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, '".$Fld_Company_ID."');"; 
								echo $compteur."-->".$req."<br><br>";
