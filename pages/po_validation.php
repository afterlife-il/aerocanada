...
<?php include "after_nav.php"; ?>
         ...
         <div id="page-wrapper">
			...
			<div class="row">
				...
				<div class="col-lg-7">
				<span style="color:#A7142A;font-weight: bold;">DETAILS RFQ</span>
				...
				<?php 
											...
											{
														$sqlverifquote="SELECT * FROM tbl_RFQ_3 WHERE Fld_RFQ_ID='".$data['Fld_RFQ_ID']."' AND id_tbl_rfq1='".$data["ID"]."'";
														$resultverifquote = mysql2_query($sqlverifquote);
														$dataverifquote = mysqli_fetch_array($resultverifquote);
														echo "<tr>";
															//recuperation PN ***********************
															$sqlrn="SELECT Fld_Part_Nbr,Fld_Part_Desc FROM tbl_Parts WHERE Fld_Part_ID='".$data['Fld_Part_ID']."'";
															...
															$sqlc="SELECT * FROM tbl_Condition WHERE Fld_Condition_ID='".$data['Fld_Condition_ID']."'";
															
															$reqc = mysql2_query($sqlc);
															$datac = mysqli_fetch_array($reqc);
															//fIN recuperation condition 
															
														echo "<td>".$datarn["Fld_Part_Nbr"]."</td>";
														echo "<td>".$datarn["Fld_Part_Desc"]."</td>";
														echo "<td>".$data["Fld_Qty"]."</td>";
														echo "<td>".$datac['Fld_Condition_Text']."</td>";
														echo "</tr>";
											}
			...
			<div class="row">
					<div class="col-lg-2">
					<span style="font-size:45px;<?php if($Fld_Phase=='0') echo "color:#A7142A;";else echo "color:#333;";?>">1</span>CUSTOMER<br> 
					<!--PO ACKNOWLEDGMENT-->
					</div>
			...
			<div class="row">
                <div class="col-lg-12">
							 ...
							 <div class="form-row">
								<div class="col-lg-2">
									<div class="form-group">
                                            <label>ACI PO#</label>
											<?php 
											$sqlracipo="SELECT Fld_ACI_PO FROM tbl_PO_Customer,Tbl_Customer_PO_Follow_UP where tbl_PO_Customer.Fld_PO=Tbl_Customer_PO_Follow_UP.Fld_PO AND Tbl_Customer_PO_Follow_UP.Fld_RFQ_ID='".$_GET['Fld_RFQ_ID']."'";
											// echo $sqlracipo;
											
											$reqracipo = mysql2_query($sqlracipo);
											$dataracipo = mysqli_fetch_array($reqracipo);
											?>
									<input class="form-control" name="Fld_ACI_PO" placeholder="PO #" value="<?php echo $dataracipo['Fld_ACI_PO'];?>" required>
									</div>
								</div>
							 </div>
							</div>
							...
							<div class="row">
								<div class="form-row">
									<div class="col-lg-2">
                                    	...
                                    	<select class="form-control" id="idsuppliers" onchange="javascript:recupinfosuppliers();">	
					...
					while($datarfq2 = mysqli_fetch_array($reqrfq2))
					{
											...
											echo "<option value='".$datarfq2['Fld_Supplier_ID']."'>".$datarn['Fld_Company_Name']."</option>";
					}
?>									
                                  </select>
							<!--END afficher quotatios supplier pour rfqid-->
								...
								<div class="form-row" id='blocidsuppliers'>
									<div class="col-lg-1" id='dividsuppliers'>
										<div class="form-group">
                                            <label>PN</label>
										...
										<div class="form-group">
                                            <label>SN</label>
										<input class="form-control" name="Fld_ACI_PO_NBR" placeholder="SN" required>
										</div>
										<div class="form-group">
                                            <label>QTY</label>
										<input class="form-control" name="Fld_ACI_PO_NBR" placeholder="QTY" required>
										</div>
								</div>
							 </div>
...
<!--*******************************************************************************-->
<!--*******************************************************************************-->
<!--Recuperation info supplier-->

function recupinfosuppliers()
{
var idsuppliers = document.getElementById("idsuppliers").value;

var blocidsuppliers=document.getElementById('blocidsuppliers');
  
blocidsuppliers.style.display='inline';
...
var xhr=null;
        ...
        else if (window.ActiveXObject)
        {
            ...
            xhr.open("POST", "recup_info_suppliers.php?id="+idsuppliers+"&Fld_RFQ_ID=<?php echo $_GET['Fld_RFQ_ID'];?>", true);/*si jamais je veux recuperer les infos sous form de get je met les infos dans le lien cad ajax.php?variable=...*/
            xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
            xhr.onreadystatechange = function() { open_idsuppliers(xhr,idsuppliers); };
            xhr.send("id="+idsuppliers);/*si je veux mettre la variable sous forme de post je la met la*/
  
}
function open_idsuppliers(xhr,id)
{
if (xhr.readyState==4)
    {
    document.getElementById('dividsuppliers').innerHTML='<div id="'+id+'" align="center">';
         var resp3;
        resp3 = xhr.responseText;
        document.getElementById('dividsuppliers').innerHTML+=resp3;
    document.getElementById('dividsuppliers').innerHTML+='</div>';
	document.location.href="#wrapper";//je redirige le lien vers le haut de la banniere (l'ancre haut wrapper)
    }
}
<!--Fin Recuperation info supplier-->
<!--*******************************************************************************-->
<!--*******************************************************************************-->
