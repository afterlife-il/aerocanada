...
//*****tbl_Parts*************  Fld_Part_ID  Fld_Part_Nbr  Fld_Part_Desc  Fld_Part_MFG  Fld_Part_MFG_Old  Fld_AC_ID  Fld_Old_LP  Fld_Part_List_Price  Fld_Part_Price_Currency_ID  Fld_Part_LP_Date  Fld_Remark status alt_pn Fld_Add_PN_Date aci_contact_entry ata_chapter   cage_code    essentiality_category_id    nha moq
		...
		if ($varoy=='1') $AC_Type=$val;
		if ($varoy=='2') $MPN=$val;
		if ($varoy=='3') $DESIGNATION=$val;
		if ($varoy=='4') $ATA=$val;
		if ($varoy=='5') $CAPABILITY=$val;
		if ($varoy=='6') $PMA=$val;
		if ($varoy=='7') $DOA=$val;
		if ($varoy=='8') $DER=$val;
		if ($varoy=='9') $Code_OEM=$val;
		if ($varoy=='10') $Design_OEM=$val;
				...
				if (0<$nb_resultats) {	
								...
								echo $existvar;
								echo "exist";		
								$req="INSERT INTO `tbl_capa_list` (`id_capa_list`, `Fld_Part_ID`, `pn`, `descriptioin`, `aircraft`, `ata`, `capability`, `pma`, `doa`, `der`, `code_oem`, `design_oem`, `id_company`) VALUES (NULL, '".$dataemp['Fld_Part_ID']."', '".$MPN."', '".$DESIGNATION."', '".$AC_Type."', '".$ATA."', '".$CAPABILITY."', '".$PMA."', '".$DOA."', '".$DER."', '".$Code_OEM."', '".$Design_OEM."', '2855');";	
							$requete = mysql2_query($req);								
								...
								$req2="INSERT INTO `tbl_capa_list` (`id_capa_list`, `Fld_Part_ID`, `pn`, `descriptioin`, `aircraft`, `ata`, `capability`, `pma`, `doa`, `der`, `code_oem`, `design_oem`, `id_company`) VALUES (NULL, '".$lastid."', '".$MPN."', '".$DESIGNATION."', '".$AC_Type."', '".$ATA."', '".$CAPABILITY."', '".$PMA."', '".$DOA."', '".$DER."', '".$Code_OEM."', '".$Design_OEM."', '2855');";	
