...
Status: deployment topology validated locally on 2026-07-15; server deployment pending.
...
Validated locally: Linux image builds, all five health checks, migrations 001-003 and checksums, idempotent tenant seed, same-origin health/OpenAPI, staging login, Company 360 creation/read, and PostgreSQL persistence after API restart. Authentication users and sessions remain in memory; an API restart requires re-login. This is persistent staging, not production.
