...
- 2026-07-15: Deployed the public PostgreSQL-only Part and Stock workspaces from `3c9480d`. `/parts/` and `/stock/internal/` use core persistent APIs, URL-state search/filter/sort/reset/pagination and query-addressable details; they label imported versus manual/staging records and state that legacy Stock migration is incomplete. Public acceptance returned 8 Parts and 1 Stock row, unauthenticated APIs 401, no `part-1`/`stock-1` fixture IDs in page HTML, and required routes 200. Commercial/Documents/service sections remain explicit boundaries with no fixture rows. Only web was recreated.
- 2026-07-15: Deployed the OAuth/OIDC configuration foundation and truthful login state from `2e8b1d0`. Google, Microsoft, Apple and LinkedIn expose safe configuration status plus state, nonce and PKCE-S256 request primitives; no callback or token flow is activated without external credentials. Public validation found four providers, zero configured, four explicit `Not configured for this staging environment` messages, password login 200 and healthy routes. The login UI also supports the deployed TOTP/recovery challenge. No LinkedIn scraping or unofficial automation exists.
- 2026-07-15: Deployed the MFA and phone-enrollment foundation from `ae5a2c9` after backup `/opt/ready2go/saas-aviation/backups/pre-mfa-20260715T151639Z`. Migration 006 adds encrypted tenant-bound TOTP factors, hashed one-use recovery codes, E.164 phone factors, expiring five-attempt OTP challenges and expanded authentication audit categories. Public acceptance proved CSRF enforcement, authenticated TOTP enrollment, phone request/verification without a code in the response, and zero temporary factor residue; owner MFA remained disabled. Only the API was recreated. The full suite passed, including PostgreSQL 22/22 with zero skips, typecheck, lint and build. No external SMS provider is configured.
...
The focused correction now removes Company sample imports and initial props from the persistent Companies route, disables sample Company detail generation in persistent builds, gives retained demo fixtures explicit `demo-co-*` identifiers, and defaults the containerized export to root-relative assets. Local validation passed the full tests, PostgreSQL 23/23 with zero skips, typecheck, lint, build, and artifact scans with zero occurrences of the three former IDs. Server cleanup was limited to regenerable BuildKit, npm, and unused pnpm-store cache; free space reached `15,546,359,808` bytes before the web-only rebuild gate. Web `2ff9534bfc1e83c41a8755d7b54d90fcbd24881d` is deployed with image `sha256:b98a6b81ac7c3a71c39bc54600bde76cd9adae04b452d2017a4257b960eb4e33`; public fixture, asset, route, auth, CRUD, restart-persistence and protected-system checks passed. Visual browser initialization failed, so screenshots, browser-console and hydration-warning status remain unverified.
