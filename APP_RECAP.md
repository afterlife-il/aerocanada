...
- 2026-07-15: Reproduced the owner-visible Company API failure from public web logs. The browser requested PostgreSQL Companies without a bearer session and received the correct 401, while the persistent UI incorrectly retained pre-rendered fixture Companies and displayed a generic error. The frontend now starts persistent Company state empty, clears records on failure, gives explicit sign-in/retry actions, redirects login to the canonical trailing-slash route, and never uses fixtures in persistent mode. API responses now carry safe correlation IDs. Local standard and PostgreSQL tests passed with zero PostgreSQL skips; deployment is pending this focused commit.
