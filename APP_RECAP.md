...
- 2026-07-15: Implemented the local persistent password/session foundation. Migration 005 adds tenant users, salted scrypt credentials, failed-attempt lockout, persistent session digests and authentication audit events. Browser login uses HttpOnly/Secure/SameSite cookies with CSRF validation; logout and revoke-all are persistent. PostgreSQL tests cover password non-reversibility, session continuity through provider restart, current/all-session revocation, lockout and cookie CSRF. Phone OTP, TOTP and OAuth remain explicit subsequent work; deployment is pending.
