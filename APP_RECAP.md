...
- 2026-07-15: Replaced the public persistent Part and Stock fixture workspaces locally. `/parts/` and `/stock/internal/` now read only core PostgreSQL APIs, support URL-state search/filter/sort/reset/pagination and query-addressable details, expose loading/error/empty states, label imported versus manual/staging records, and state that legacy Stock migration is incomplete. Commercial/Documents/service sections are explicit boundaries with no fixture rows. Legacy static fixture detail paths are disabled in persistent mode. Tests verify no public Part/Stock page imports sample data. Deployment is pending.
