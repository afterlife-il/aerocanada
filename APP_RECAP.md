...
- 2026-07-15: Added the local OAuth/OIDC configuration foundation and truthful login state. Google, Microsoft, Apple and LinkedIn definitions now have safe configuration status plus state, nonce and PKCE-S256 request primitives; no callback or token flow is activated without external credentials. The login UI labels every provider `Not configured for this staging environment` and supports the deployed TOTP/recovery login challenge. Tests cover disabled status and state/nonce/PKCE. Deployment is pending; no LinkedIn scraping or unofficial automation exists.
